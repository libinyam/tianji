...
export default function SearchModal({ open, onClose }: SearchModalProps) {
  ...
  return (
    <Dialog
      ...
      opaque
    >
      <div className="overflow-hidden rounded-lg border border-void-600 bg-void-900 shadow-sm">
        {/* 搜索栏 */}
        <div className="flex items-center gap-3 border-b border-void-600 px-4 py-3">
          {loading ? (
          ...
          <input
            ...
            aria-label="搜索"
            className="flex-1 bg-transparent text-sm text-parchment-100 outline-none placeholder:text-mist-500"
          />
          ...
          <button
            onClick={onClose}
            aria-label="关闭"
            className="text-mist-500 hover:text-parchment-100"
          >
            <X size={18} />
          </button>
        </div>
          ...
          {searched ? (
            loading ? (
              <div className="py-12 text-center text-sm text-mist-500">
                <Loader2 size={20} className="mx-auto mb-2 animate-spin text-tian-500" />
                正在搜索…
              </div>
            ) : results.length === 0 ? (
              <div className="py-12 text-center text-sm text-mist-500">
              ...
              <div className="space-y-1">
                <p className="px-3 py-1.5 text-xs text-mist-500">找到 {results.length} 条结果</p>
                {results.map((r) => {
                  ...
                  return (
                    <button
                      ...
                      className="group flex w-full items-start gap-3 rounded-lg px-3 py-2.5 text-left transition-colors hover:bg-void-700"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {r.excerpt && (
                          <p className="mt-0.5 truncate text-xs text-mist-500">{r.excerpt}</p>
                        )}
            ...
            <div>
              ...
              {hotList.length === 0 ? (
                <div className="py-8 text-center text-sm text-mist-500">暂无热门内容</div>
              ) : (
                  ...
                  {hotList.map((item, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className="group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors hover:bg-void-700"
                      >
                        <span
                          className={`w-5 shrink-0 text-center font-mono text-sm font-bold ${rankColor}`}
                        >
                          {idx + 1}
                        </span>
                        <Icon size={14} className={`shrink-0 ${color}`} />
                        <span className="min-w-0 flex-1 truncate text-sm text-parchment-100 group-hover:text-tian-500">
                          {item.title}
                        </span>
