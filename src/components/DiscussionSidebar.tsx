    ...
    <aside className="hidden w-[260px] shrink-0 lg:block">
      <div className="sticky top-4 space-y-4">
        ...
        <div className="card-surface p-4">
          ...
          <Link
            to="/about"
            className="mt-2 inline-flex items-center gap-1 text-xs text-tian-500 transition-colors hover:text-tian-600"
          >
        ...
        <nav className="card-surface overflow-hidden p-2">
          <div className="px-2 py-1 text-xs font-medium text-mist-500">分区</div>
          {SECTIONS.map((s) => {
            const Icon = s.icon;
        ...
        {section === "academic" && hotTags.length > 0 && (
          <nav className="card-surface p-2">
            <div className="px-2 py-1 text-xs font-medium text-mist-500">热门标签</div>
            <button
        ...
        {user && followedTags.length > 0 && (
          <nav className="card-surface p-2">
            <div className="flex items-center gap-1.5 px-2 py-1 text-xs font-medium text-mist-500">
            ...
            {followedTags.map((tag) => (
              <Link
                ...
                className="flex items-center gap-2 rounded-md px-2.5 py-1.5 text-sm text-mist-400 transition-colors hover:bg-void-700 hover:text-parchment-100"
              >
                <span
                  className="h-2 w-2 shrink-0 rounded-full"
        ...
        <div className="card-surface px-4 py-3">
          <div className="flex items-center gap-1.5 text-xs font-medium text-parchment-100">
            <Compass size={13} className="text-tian-500" /> 第一次来天玑？
          </div>
          <ol className="mt-2 space-y-2">
            {ONBOARDING_STEPS.map((s) => (
              <li key={s.step}>
                <Link
                  to={s.link}
                  className="group flex items-center gap-2 text-xs text-mist-400 transition-colors hover:text-tian-500"
                >
                  <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-void-700 text-[10px] text-mist-500">
                    {s.step}
                  </span>
                  {s.text}
                </Link>
        ...
        <div className="card-surface px-2 py-1">
          <div className="px-2 py-1 text-xs font-medium text-mist-500">其他模块</div>
          {MODULES.map((m) => {
            const Icon = m.icon;
            return (
              <Link
                key={m.to}
                to={m.to}
                className="group flex items-center gap-3 rounded-md px-2.5 py-2 transition-colors hover:bg-void-700"
              >
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-void-600 bg-void-700 text-tian-500">
                  <Icon size={14} strokeWidth={1.5} />
                </span>
                <span className="min-w-0">
                  <span className="block text-xs font-medium text-parchment-100 transition-colors group-hover:text-tian-500">
                    {m.label}
                  </span>
        ...
        {hot.length > 0 && (
          <div className="card-surface px-4 py-3">
            <div className="flex items-center gap-1.5 text-xs font-medium text-parchment-100">
              <Flame size={13} className="text-tian-500" /> 热门讨论
            </div>
              ...
              {hot.map((q, i) => (
                <li key={q.id}>
                  <Link to={`/discussion/${q.id}`} className="group flex items-start gap-2">
                    <span
                      className={`text-xs font-medium ${i < 3 ? "text-tian-500" : "text-mist-500"}`}
                    >
                      {i + 1}
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="line-clamp-2 text-xs leading-snug text-mist-300 transition-colors group-hover:text-tian-500">
                        {q.title}
                      </span>
