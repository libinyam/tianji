    ...
    <aside className="hidden space-y-3 lg:sticky lg:top-20 lg:block">
      ...
      <div className="card-surface p-4">
        <div className="flex items-center gap-2">
          <Sparkles className="h-3.5 w-3.5 text-star-400" strokeWidth={1.5} />
          <span className="heading-display text-sm text-parchment-50">
            天<span className="text-star-400">玑</span>
          </span>
          <span className="text-[11px] text-mist-500">跨专业共创社区</span>
        </div>
        ...
        <Link
          to="/about"
          className="mt-2 inline-flex items-center gap-1 text-[11px] text-star-400 transition-colors hover:text-star-300"
        >
          了解天玑 <ArrowUpRight size={11} />
        </Link>
      </div>

      {/* 新手引导 - 首屏只保留必要卡片，降低视觉重量 */}
      <div className="card-surface px-4 py-3">
        <div className="flex items-center gap-1.5 text-[11px] font-medium text-parchment-100">
          <Compass size={12} className="text-star-400" /> 第一次来天玑？
        </div>
        <ol className="mt-2 space-y-1.5">
          {ONBOARDING_STEPS.map((s) => (
            <li key={s.step}>
              <Link
                to={s.link}
                className="group flex items-center gap-2 text-[11px] text-mist-400 transition-colors hover:text-star-400"
              >
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block text-[11px] font-medium text-parchment-100 transition-colors group-hover:text-star-400">
                  {m.label}
                </span>
                <span className="block truncate text-[10px] text-mist-500">{m.desc}</span>
              </span>
            </Link>
          );
        })}
      ...
      {hot.length > 0 && (
        <div className="card-surface px-4 py-3">
          <div className="flex items-center gap-1.5 text-[11px] font-medium text-parchment-100">
            <Flame size={12} className="text-star-400" /> 热门讨论
          </div>
          <ul className="mt-2 space-y-2">
            {hot.map((q, i) => (
              <li key={q.id}>
                <Link to={`/discussion/${q.id}`} className="group flex items-start gap-2">
                  <span className={`font-mono text-[11px] ${i < 3 ? "text-star-400" : "text-mist-500"}`}>
                    {i + 1}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="line-clamp-2 text-[11px] leading-snug text-mist-300 transition-colors group-hover:text-star-400">
                      {q.title}
                    </span>
