    ...
    >
      <div className="flex max-h-[85vh] flex-col">
        ...
        <div className="relative shrink-0">
          <span className="text-xs font-medium text-tian-500">新建协作项目</span>
          <h3
            id="workshop-create-dialog-title"
            className="mt-1 heading-display text-2xl text-parchment-50"
          >
            发起共创
          </h3>
          <p className="mt-2 text-sm text-mist-400">创建大纲，邀请其他学习者共同书写教材或论文。</p>
          {/* #98 草稿恢复提示 */}
          {restored && (
            <div className="mt-3 flex items-center justify-between rounded-md border border-void-600 bg-void-700 px-3 py-2 text-xs text-mist-400">
          ...
          <div className="flex-1 space-y-5 overflow-y-auto pr-1">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex flex-1 items-center gap-2 rounded-lg border p-3 text-sm transition-colors ${
                    type === "教材"
                      ? "border-tian-500 bg-void-700 text-tian-600"
            ...
            <div>
              ...
              <input
                ...
                placeholder="项目名称"
                className="w-full rounded-lg border border-void-600 bg-void-950 px-3 py-2.5 text-sm text-parchment-100 placeholder:text-mist-500 focus:border-tian-500 focus:outline-none focus:ring-1 focus:ring-tian-500"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full resize-y rounded-lg border border-void-600 bg-void-950 p-3 text-sm leading-relaxed text-parchment-100 placeholder:text-mist-500 focus:border-tian-500 focus:outline-none focus:ring-1 focus:ring-tian-500"
                maxLength={3000}
              />
            </div>
            ...
            {/* 大纲 */}
            <div>
              <label className="mb-1.5 block text-xs text-mist-400">大纲（章节列表）*</label>
              <div className="space-y-3">
                {outline.map((ch, i) => (
                  <div
                    key={ch.id}
                    className="flex gap-2 rounded-lg border border-void-600 bg-void-700 p-3"
                  >
                    <span className="mt-2 text-xs text-mist-500">第{i + 1}章</span>
                    <div className="flex-1 space-y-2">
                      <input
                        ...
                        placeholder="章节标题"
                        maxLength={150}
                        className="w-full rounded-md border border-void-600 bg-void-950 px-2.5 py-1.5 text-sm text-parchment-100 placeholder:text-mist-500 focus:border-tian-500 focus:outline-none"
                      />
                      <input
                        name="chapter-brief"
                        type="text"
                        ...
                        placeholder="简述（可选）"
                        maxLength={200}
                        className="w-full rounded-md border border-void-600 bg-void-950 px-2.5 py-1.5 text-xs text-mist-300 placeholder:text-mist-500 focus:border-tian-500 focus:outline-none"
                      />
                    </div>
                    {outline.length > 1 && (
                      <button
                        ...
                        className="mt-1 text-mist-500 transition-colors hover:text-red-500"
                      >
                        <Trash2 size={15} />
                      </button>
                    )}
              ...
              <button
                type="button"
                onClick={addChapter}
                className="mt-2 flex items-center gap-1 text-xs text-tian-500 transition-colors hover:text-tian-600"
              >
                <Plus size={13} /> 添加章节
              </button>
            </div>
