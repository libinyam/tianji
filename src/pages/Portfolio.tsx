...
export default function Portfolio() {
  ...
  return (
    <>
      ...
      >
        <Link to="/growth" className="btn-primary inline-flex items-center gap-2">
      ...
      <section className="container-tj py-8">
        {loading ? (
          ...
          <>
            {/* 作品集统计 */}
            <div className="mb-6 flex flex-wrap items-center gap-6 rounded-lg border border-void-600 bg-void-700 p-4">
              <div>
                <span className="heading-display text-2xl text-parchment-50">{works.length}</span>
                <span className="ml-2 text-sm text-mist-400">个作品</span>
              </div>
              <div className="h-8 w-px bg-void-600" />
              <div>
                <span className="heading-display text-2xl text-parchment-50">
            ...
            <div className="divide-y divide-void-600 rounded-lg border border-void-600 bg-void-800">
              {works.map((work) => (
                <Link
                  key={work.id}
                  to={`/workshop/${work.id}`}
                  className="group flex flex-col p-5 transition-colors hover:bg-void-700"
                >
                  {/* 标题行 */}
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="heading-display text-lg text-parchment-50">{work.title}</h3>
                    <span className="pill-blue shrink-0">{work.type}</span>
                  </div>
                    ...
                    <div className="mt-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {work.outline.slice(0, 4).map((ch) => (
                          <span key={ch.id} className="pill">
                            {ch.title.length > 12 ? ch.title.slice(0, 12) + "…" : ch.title}
                          </span>
                        ))}
                        {work.outline.length > 4 && (
                          <span className="pill">+{work.outline.length - 4}</span>
                        )}
                      </div>
                    </div>
                  )}
