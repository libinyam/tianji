...
export default function Library() {
  ...
  useSEO({
    title: "资源库",
    description:
      "天玑学习资源库 -- 精选 AI 工具实战、编程基础、项目实战与基础理论书单，支持上传分享与社区评价。",
        ...
        <div className="container-tj flex h-12 items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs text-mist-500">
              {filtered.length} 份资源
              {filtered.length !== allBooks.length && ` / 共 ${allBooks.length} 份`}
            </span>
          </div>
          <button
      ...
      <section className="container-tj py-6">
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative max-w-xs flex-1">
            <Search
              size={14}
              className="absolute left-2.5 top-1/2 -translate-y-1/2 text-mist-500"
            />
            <input
              name="search"
              value={query}
              ...
              className="w-full rounded-md border border-void-600 bg-void-900 py-1.5 pl-8 pr-3 text-xs text-parchment-100 placeholder:text-mist-500 focus:border-tian-500 focus:outline-none"
            />
          </div>
            ...
            <div className="flex items-center gap-0.5">
              {CATEGORIES.map((c) => (
                <button
                  ...
                  className={`rounded px-2 py-1 transition-colors ${category === c ? "font-medium text-tian-500" : "text-mist-500 hover:text-mist-300"}`}
                >
                  {c}
                </button>
              ))}
            ...
            <div className="flex items-center gap-1">
              {SORTS.map((s) => (
                <button
                  key={s}
                  onClick={() => setSort(s)}
                  className={`rounded px-2 py-1 transition-colors ${sort === s ? "font-medium text-tian-500" : "text-mist-500 hover:text-mist-300"}`}
                >
                  {s}
                </button>
              ))}
        ...
        {!loading && loadError && (
          <div className="rounded-lg border border-dashed border-red-300 py-10 text-center">
            <p className="text-mist-300">资源加载失败，可能是网络或登录态过期</p>
            <button onClick={loadBooks} className="btn-secondary mt-4">
              重新加载
            </button>
          </div>
        )}
        ...
        {!loading && !loadError && filtered.length === 0 && (
          <div className="rounded-lg border border-dashed border-void-600 py-10 text-center text-mist-400">
            未找到匹配的书目，试试其他关键词。
          </div>
        )}
      </section>
