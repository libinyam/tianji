...
const uid =
  uidArg !== -1 && uidArg + 1 < args.length ? args[uidArg + 1] : process.env.SEED_AUTHOR_UID;
...
const route1Books = [
  {
    ...
    tags: ["C++", "编程基础", "入门"],
    summary:
      "C++ 经典入门教材，从基础语法到面向对象、模板、标准库全覆盖。适合有编程基础的同学系统学习 C++，每章配有练习题。",
    toc: [
      "第1章 开始",
      "第2章 变量和基本类型",
      "第3章 字符串、向量和数组",
  ...
  },
  {
    ...
    tags: ["C++", "进阶", "最佳实践"],
    summary:
      "C++ 进阶必读，55 条最佳实践，涵盖资源管理、模板、泛型编程等。每条都有正反例对比，帮你写出更安全高效的 C++ 代码。",
    toc: [
      "第一章 让自己习惯 C++",
      "第二章 构造/析构/赋值运算",
      "第三章 资源管理",
  ...
  },
  {
    ...
    tags: ["C++", "CLI", "项目实战", "笔记"],
    summary:
      "从零开始用 C++ 构建一个命令行笔记管理工具，涵盖文件 I/O、字符串处理、简单解析器设计。适合学完 C++ Primer 后的第一个实战项目。",
    toc: [
      "1. 项目规划与环境搭建",
      "2. 文件读写与存储格式",
      "3. 命令行参数解析",
...
const route1Ideas = [
  {
    ...
    summary:
      "学完 C++ 基础后想做一个实战项目：命令行笔记工具，支持 Markdown 存储、标签分类、全文搜索。最终目标是把它变成个人的 AI 知识系统后端——笔记可被检索、关联、问答。",
    topic: "把 C++ 学成真项目",
    tags: ["C++", "CLI", "笔记", "知识管理"],
  },
  {
    ...
    summary:
      "很多人学完语法就卡住，不知道做什么。我整理了一个递进式项目清单：计算器 → 待办列表 → 笔记系统 → 简易数据库，每一步只增加一个核心概念，避免一上来就做太大的东西。",
    topic: "学习路线设计",
    tags: ["C++", "学习路线", "项目"],
  },
];
...
const route1Workshop = {
  ...
  type: "教材",
  description:
    "协作编写「用 C++ 构建命令行笔记系统」的完整教程，从环境搭建到全文搜索。目标是产出一套新手跟着做就能跑起来的项目教程。",
  content:
  ...
  outline: [
    { id: "ch1", title: "项目规划与环境搭建", brief: "CMake 项目结构、依赖管理、开发环境配置" },
    {
      id: "ch2",
      title: "文件读写与存储格式",
      brief: "笔记文件格式设计、index.json 索引、CRUD 实现",
    },
    {
      id: "ch3",
      title: "命令行参数解析",
      brief: "cxxopts 集成、子命令设计（add/list/search/export）",
    },
...
const route2Books = [
  {
    ...
    summary:
      "面向新手的 GitHub Actions 教程，从第一个 workflow 到自动部署。包含天玑项目真实的 CI 配置案例：lint、test、audit、分支保护。",
    toc: [
      "1. GitHub Actions 基础概念",
      "2. 第一个 workflow",
      "3. CI：lint + test + type check",
      "4. CD：自动部署到 Vercel",
      "5. 分支保护与 PR 流程",
      "6. 实战：天玑项目的 CI 配置",
    ],
  },
  {
    ...
    summary:
      "腾讯云 CloudBase 全栈部署指南：静态托管、云函数、NoSQL 数据库、安全规则。以天玑项目为案例，覆盖从环境创建到生产部署的完整流程。",
    toc: [
      "1. CloudBase 环境创建",
      "2. 静态托管部署",
      "3. 云函数开发与部署",
  ...
  },
  {
    ...
    summary:
      "记录用 TRAE 开发天玑项目的全过程：需求拆解、代码生成、bug 修复、测试。展示人机协作的真实工作流——人提方向，AI 辅助实现。",
    toc: [
      "1. TRAE 是什么",
      "2. 项目初始化协作",
      "3. 多模块并行开发",
...
const route2Ideas = [
  {
    ...
    summary:
      "现在 AI 应用部署门槛还是太高：前端、后端、模型 API、数据库各搞各的。想探索一套模板：Vercel 前端 + CloudBase 后端 + DeepSeek API，新项目 fork 后改改就能上线。",
    topic: "部署模板化",
  ...
  },
  {
    ...
    summary:
      "用 TRAE 开发天玑几个月后的反思：AI 最擅长的是拆解和生成，人最该做的是判断和取舍。具体来说——人定方向、定验收标准、review 代码，AI 负责实现。不要让 AI 替你做产品决策。",
    topic: "AI 协作方法论",
    tags: ["TRAE", "AI", "协作", "方法论"],
  },
];
...
const route2Workshop = {
  ...
  type: "教材",
  description:
    "协作整理全栈应用的部署最佳实践文档，覆盖 Vercel 前端、CloudBase 后端、安全规则、CI/CD、自定义域名。目标产出一套可复用的部署 checklist。",
  content:
    "## 目标\n\n整理一套全栈应用部署的标准化 checklist，新人照着做就能上线。\n\n## 覆盖范围\n\n1. 前端部署（Vercel）\n2. 后端部署（CloudBase 云函数 + 数据库）\n3. 安全配置（安全规则、环境变量、分支保护）\n4. CI/CD（GitHub Actions）\n5. 域名与备案\n6. 监控与告警",
  outline: [
    {
      id: "ch1",
      title: "前端部署：Vercel 配置要点",
      brief: "构建配置、环境变量、安全头、缓存策略",
    },
    {
      id: "ch2",
      title: "后端部署：CloudBase 云函数",
      brief: "云函数开发、依赖管理、环境变量、定时任务",
    },
...
async function main() {
  ...
  if (existingBooks.length > 0) {
    console.log(
      `⚠️  已发现 ${existingBooks.length} 条种子书籍，跳过书籍注入（如需重新注入请先删除）`,
    );
  } else {
